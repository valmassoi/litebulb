  ...
  render() {
    ...
    if (mode === 'all') {
      if (bulb.like)// TODO reverse
        empty = '-empty'
      likeBtn = (
        <p class="pull-right" onClick={() => likeBulb(bulb)}>
          <span class={`glyphicon glyphicon-heart${empty}`} aria-label="true" style={likeStyle} />
        </p>
    ...
    return (
      <div class="image-element-class" style={cardStyle}>
        <img
          ...
          alt="bulb"
          style={imgStyle}
          onError={(image) => this.imgError(image)}
        />
        <div style={infoStyle}>
